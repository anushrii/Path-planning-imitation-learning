...
#define isinf(x) (std::numeric_limits<double>::infinity() == x) 
...
NeighborStruct neighbors[] = {
  {1,0,1.0}, {2,1,sqrt(5.0)}, {1,1,sqrt(2.0)}, {1,2,sqrt(5.0)},
  {0,1,1.0}, {-1,2,sqrt(5.0)}, {-1,1,sqrt(2.0)}, {-2,1,sqrt(5.0)},
  {-1,0,1.0}, {-2,-1,sqrt(5.0)}, {-1,-1,sqrt(2.0)}, {-1,-2,sqrt(5.0)},
  {0,-1,1.0}, {1,-2,sqrt(5.0)}, {1,-1,sqrt(2.0)}, {2,-1,sqrt(5.0)},
};
...
{	double rndVal;
	int temp = val;
	if ((val-double(temp))<0.5)
	{	
		rndVal = double(temp);
	}
	else 
	{	
		rndVal = double(temp+1);
	}
	return rndVal;
}
...
nNode++;
	...
	cost += A[m*jk+ik];
      ...
      double h1 = sqrt(double((iStart-i1)*(iStart-i1)+(jStart-j1)*(jStart-j1)));
